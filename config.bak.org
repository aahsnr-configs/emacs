...
(use-package org
  ...
  :hook ((org-mode . (lambda ()
                       ;; CRITICAL: Prevent Emacs from auto-indenting on RET, 
                       ;; which causes the cursor jump in source blocks.
                       (setq-local electric-indent-inhibit t)
                       
                       ;; CRITICAL: Custom RET behavior.
                       ;; Inside blocks: Use simple newline+indent (native behavior).
                       ;; Outside: Use Org's smart return (tables, headings).
                       ...
                       (evil-define-key 'normal org-mode-map (kbd "[b") #'ar/org-nav-src-prev)
                       (evil-define-key 'normal org-mode-map (kbd "]b") #'ar/org-nav-src-next)))
  ...
  :config
  ;; === Navigation Helpers ===
  (defun ar/org-nav-src-prev ()
    "Jump to the first line of the current source block content."
    (interactive)
    (org-babel-goto-src-block-head)
    (forward-line 1))

  (defun ar/org-nav-src-next ()
    "Jump to the last line of the current source block content."
    (interactive)
    (org-babel-goto-src-block-head)
  ...
  (defun hot-expand (str &optional mod)
    (let (text)
      ...
      (if (fboundp 'org-try-structure-completion)
        ...
        (progn (require 'org-tempo nil t) (org-tempo-complete-tag)))
  ...
  (dolist (pair '(("sh" . "src shell") ("el" . "src emacs-lisp")
                  ("py" . "src python") ("jpy" . "src jupyter-python")
                  ("tex" . "src latex")))
  ...
  ;; CRITICAL: Must be nil. If t, Org expects manual indentation 
  ;; and 'newline-and-indent' will behave erratically (jumping to top).
  (org-src-preserve-indentation nil)
  
  ;; Keep code flush with #+begin_src
  ...
  (org-tag-alist '(("@work" . ?w) ("@home" . ?h) ("@computer" . ?c)
                   ("@errands" . ?e) ("read" . ?r) ("meeting" . ?m)
                   ("urgent" . ?u) ("someday" . ?s))))
#+end_src

** Org On-Demand Breadcrumbs
Show the current Org headline hierarchy (Breadcrumbs) on demand.
Replaces automatic display with a manual keybinding to reduce clutter.
#+begin_src emacs-lisp
(with-eval-after-load 'org
  (defun ar/org-show-breadcrumb ()
    "Display a Doom-style breadcrumb path in the echo area.
    Format: Parent › Current
    Special states: 'Top Level' (start) and 'Bottom Level' (end).
    Does NOT show filename (redundant with modeline)."
    (interactive)
    (if (not (derived-mode-p 'org-mode))
        (message "Not in an Org buffer.")
      
      (let* (;; Styles
             (face-sep '(:inherit shadow :height 0.8))
             (face-path '(:inherit shadow))
             (face-current '(:inherit font-lock-keyword-face :weight bold))
             (sep-str (propertize " › " 'face face-sep))
             
             ;; Get path including current heading ('t')
             ;; ignore-errors handles "Before first headline" case gracefully
             (path (ignore-errors (org-get-outline-path t))))

        (cond
         ;; Case A: End of Buffer
         ((eobp)
          (message "%s" (propertize "Bottom Level" 'face face-path)))
         
         ;; Case B: Inside a hierarchy
         (path
          (message "%s"
                   (mapconcat
                    (lambda (heading)
                      ;; If it's the last item, highlight it. Otherwise dim it.
                      (if (string= heading (car (last path)))
                          (propertize heading 'face face-current)
                        (propertize heading 'face face-path)))
                    path
         ...
         ;; Case C: Top of file (no headings and not at bottom)
         (t 
          (message "%s" (propertize "Top Level" 'face face-path))))))))
#+end_src
...
(use-package treesit
  :ensure nil
  :preface
  (defun ar/setup-install-grammars ()
...
** Hideshow
Standard native folding, kept as fallback
#+begin_src emacs-lisp
...
(use-package cape
  :defer t
  :commands (cape-file cape-elisp-block cape-keyword cape-file cape-tex)
  :autoload (cape-wrap-noninterruptible cape-wrap-nonexclusive cape-wrap-buster)
  :autoload (cape-wrap-silent cape-wrap-purify)
  :init
  (add-hook 'completion-at-point-functions #'cape-elisp-block 10)
  (add-hook 'completion-at-point-functions #'cape-keyword 30)
  (add-hook 'completion-at-point-functions #'cape-file 40)  
  (add-hook 'completion-at-point-functions #'cape-dabbrev 50)
  (add-hook 'completion-at-point-functions #'cape-dict 60)
  (add-hook 'completion-at-point-functions #'cape-tex 70)
  ...
  (pretty-hydra-define version-control-hydra
    ...
    ("Magit Core"
     (("s" magit-status "status" :exit t)
      ...
      ("C" magit-commit-amend "amend" :exit t)
     ...
     "File History"
     (("t" git-timemachine-toggle "timemachine" :toggle git-timemachine-mode)
      ("n" git-timemachine-show-next-revision "next revision" :exit t)
      ("N" git-timemachine-show-previous-revision "prev revision" :exit t)
      ("w" git-timemachine-kill-abbreviated-revision "copy hash")
      ("W" git-timemachine-kill-revision "copy full hash")
      ("y" git-timemachine-show-commit "show commit" :exit t))
     
     "Hunk Navigation"
     (("j" ar/git-gutter-next-hunk "next hunk")
      ("k" ar/git-gutter-previous-hunk "prev hunk")
      ("h" git-gutter:popup-hunk "preview")
      ("a" git-gutter:stage-hunk "stage")
      ("v" git-gutter:revert-hunk "revert"))
     
     "Blame & Messages"
     (("A" ar/magit-blame-addition "blame" :exit t)
      ("M" git-messenger:popup-message "message" :exit t)
      ("o" browse-at-remote "open remote" :exit t)
      ("O" git-link "get link" :exit t))
...
(use-package markdown-mode
  ...
  (defun ar/markdown-optimize-fontification ()
    ...
    ;; Reduce how often font-lock runs
    (setq-local font-lock-multiline t)
    (setq-local font-lock-maximum-decoration 2) ;; Reduce decoration level
    
    ;; Increase chunk size for jit-lock to reduce calls
    (setq-local jit-lock-chunk-size 1000)
    
    ;; Don't fontify during input
...
Uses text properties to detect code, emphasis, and links."
  ...
  ;; ANTI-FLICKER: Throttled markup toggling to prevent rapid refontification
  (defvar ar/markdown-toggle-timer nil
    "Timer to throttle markup visibility toggling.")
  
  (defun ar/markdown-toggle-markup-at-point ()
    "Toggle markup hiding based on cursor position with throttling.
Shows markup when cursor is inside markdown elements."
    (when (derived-mode-p 'markdown-mode)
      ...
      ;; Schedule toggle with small delay to batch rapid cursor movements
                ...
                (when (buffer-live-p (current-buffer))
                  (with-current-buffer (current-buffer)
                    (if (ar/markdown-in-element-p)
                        ;; Inside element: show markup on current line only
  ...
  ;; CHECKBOX RENDERING: Make checkboxes appear filled
...
:pretty-hydra
((:title (pretty-hydra-title "LSP Mode" 'codicon "nf-cod-code" :face 'nerd-icons-blue)
  :color amaranth
  :quit-key ("q" "C-g"))
 ("Navigation"
  (("d" lsp-find-definition "definition")
   ("D" lsp-find-declaration "declaration")
   ("y" lsp-find-type-definition "type definition")
  ...
  (("w r" lsp-workspace-restart "restart")
   ...
   ("w a" lsp-workspace-folders-add "add folder")
   ("w x" lsp-workspace-folders-remove "remove folder")
   ("w b" lsp-workspace-blacklist-remove "unblacklist"))
  
  "Toggles"
  (("T h" lsp-toggle-symbol-highlight "highlight" :toggle lsp-enable-symbol-highlighting)
   ("T l" lsp-lens-mode "lens" :toggle lsp-lens-mode)
   ("T b" lsp-headerline-breadcrumb-mode "breadcrumb" :toggle lsp-headerline-breadcrumb-mode)
   ("T f" lsp-toggle-on-type-formatting "on-type format" :toggle lsp-enable-on-type-formatting)
   ("T s" lsp-toggle-signature-auto-activate "signature" :toggle lsp-signature-auto-activate))
  
  "Diagnostics"
  (("e l" lsp-treemacs-errors-list "error list" :exit t)
   ("e n" flycheck-next-error "next error")
   ("e p" flycheck-previous-error "prev error")
   ("e c" flycheck-clear "clear")
   ("e b" flycheck-buffer "check buffer")
   ("e e" consult-flycheck "consult errors" :exit t))
  
  "Actions"
  (("TAB" lsp-treemacs-symbols "symbols tree" :exit t)
   ("C" lsp-treemacs-call-hierarchy "call hierarchy" :exit t)
   ("t" lsp-treemacs-type-hierarchy "type hierarchy" :exit t)
   ("u" lsp-ui-hydra/body "LSP UI" :exit t)
   ("?" lsp-describe-session "info" :exit t))))
...
:pretty-hydra
((:title (pretty-hydra-title "LSP UI" 'codicon "nf-cod-symbol_color" :face 'nerd-icons-green)
  :color amaranth
  :quit-key ("q" "C-g"))
 ("Peek"
  (("p d" lsp-ui-peek-find-definitions "definitions")
   ("p r" lsp-ui-peek-find-references "references")
   ("p i" lsp-ui-peek-find-implementation "implementation")
   ("p s" lsp-ui-peek-find-workspace-symbol "workspace symbol")
   ("p j" lsp-ui-peek-jump-forward "jump forward")
   ("p k" lsp-ui-peek-jump-backward "jump backward"))
  
  "Doc"
  (("d s" lsp-ui-doc-show "show")
   ("d h" lsp-ui-doc-hide "hide")
   ("d g" lsp-ui-doc-glance "glance")
   ("d f" lsp-ui-doc-focus-frame "focus")
   ("d u" lsp-ui-doc-unfocus-frame "unfocus"))
  ...
  (("D e" (progn
   ...
   ("D i" (setq lsp-ui-doc-include-signature (not lsp-ui-doc-include-signature))
    "signature" :toggle lsp-ui-doc-include-signature)
   ("D h" (setq lsp-ui-doc-header (not lsp-ui-doc-header))
    "header" :toggle lsp-ui-doc-header))
  
  "Doc Position"
  (("D t" (setq lsp-ui-doc-position 'top) "top")
   ("D b" (setq lsp-ui-doc-position 'bottom) "bottom")
   ("D p" (setq lsp-ui-doc-position 'at-point) "at point"))
  ...
  (("s e" (progn
   ...
   ("s a" (setq lsp-ui-sideline-show-code-actions (not lsp-ui-sideline-show-code-actions))
  ...
  (("I" lsp-ui-imenu "imenu" :exit t)
   ...
   ("m" lsp-mode-hydra/body "LSP Mode" :exit t))))
...
(defconst org-babel-lang-list
  '("go" "python" "ipython" "jupyter-python" "ruby" "js" "css" "sass" "c" "rust" "java" "cpp" "c++" "shell")
...
(use-package org-src-context
  ...
  :config
  (defun ar/disable-apheleia-in-context-buffers ()
    (when (bound-and-true-p org-src-context-mode)
      (apheleia-mode -1)))
  (add-hook 'org-src-mode-hook #'ar/disable-apheleia-in-context-buffers 100))
#+end_src
...
(use-package flycheck
  ...
  ;; Faster error display in echo area
...
** Character Information at Point
#+begin_src emacs-lisp
(defun ar/show-char-info ()
  "Show detailed character information at point in echo area.
Wrapper around `what-cursor-position' with prefix arg implied."
  (interactive)
...
(use-package lsp-pyright
  ...
  (lsp-disabled-clients '(ruff ruff-lsp)) ;; Built-in LSP way to disable clients
  :hook ((python-mode python-ts-mode) . lsp-deferred))

;; Configure Ruff for Flycheck & Apheleia (Non-redundant)
(with-eval-after-load 'python
  ;; 1. Flycheck: Define & Chain
  (with-eval-after-load 'flycheck
    (flycheck-define-checker python-ruff
      ...
      :standard-input t
      :error-patterns
      ((warning line-start (file-name) ":" line ":" (optional column ":") " " (id (one-or-more (any alpha)) (one-or-more digit)) " " (message) line-end)
       (error line-start (file-name) ":" line ":" (optional column ":") " " (id (one-or-more (any alpha)) (one-or-more digit)) " " (message) line-end))
      :modes (python-mode python-ts-mode))
    (add-to-list 'flycheck-checkers 'python-ruff)
    (with-eval-after-load 'lsp-diagnostics
      (flycheck-add-next-checker 'lsp 'python-ruff)))

  ;; 2. Apheleia: Define & Configure
  ...
  ;; 3. Disable LSP formatting (Local Hook)
...
(use-package tex
  ...
  ;; Folding & Prettification
  (TeX-fold-auto t)
  (font-latex-fontify-script 'multi-level)
  (font-latex-fontify-sectioning 1.2)

  ;; Viewer Configuration (PDF Tools)
...
(with-eval-after-load 'org
  ...
  ;; 3. Visual Settings (Transparent background)
  (setq org-format-latex-options
        (plist-put org-format-latex-options :background "Transparent"))
  (setq org-format-latex-options
        (plist-put org-format-latex-options :foreground "Auto"))

  ;; 4. Define lualatex-png process
  ...
  (when (bound-and-true-p lsp-mode)
    ...
    (setq-local completion-at-point-functions
                (list (cape-capf-super
                       #'lsp-completion-at-point      ; LSP intelligence (citations, refs)
...
(use-package general
  ...
  (ar/global-leader
    ...
    "c e" '(consult-flycheck :wk "Errors")
