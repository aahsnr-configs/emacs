...
(use-package treesit
  ...
  ;; Use 'file-truename' to resolve /var/home symlinks on Silverblue.
  ...
  ;; 3. Register in load-path immediately so Emacs finds existing grammars.
  ;; Setting this as the FIRST item ensures 'treesit-install-language-grammar'
  ;; uses it as the default output directory if no argument is provided.
  ...
  ;; 5. Auto-Install Helper (Runs on hook)
  ;; Checks for MISSING grammars and installs them silently.
    ...
    (dolist (grammar treesit-language-source-alist)
      (let ((lang (car grammar)))
        (unless (treesit-language-available-p lang)
          (treesit-install-language-grammar lang ar/treesit-grammar-dir)))))

  ;; 6. Update Helper (Interactive)
  ;; Forces re-installation/compilation from source to get updates.
  (defun ar/treesit-update-grammars (languages)
    "Update Tree-sitter grammars for LANGUAGES.
    If called interactively, prompt for languages to update.
    Press RET without selection to update ALL grammars."
    (interactive (list
                  (let ((choices (mapcar #'car treesit-language-source-alist)))
                    (let ((selected (completing-read-multiple "Update grammars (empty for all): " choices)))
                      (if (or (null selected) (string= (car selected) ""))
                          choices ;; Default to all if empty
    ...
    (dolist (lang languages)
      ...
      (condition-case err
          (progn
            (treesit-install-language-grammar lang ar/treesit-grammar-dir)
            (message "Successfully updated %s" lang))
        (error (message "Failed to update %s: %s" lang (error-message-string err))))))

  ;; 7. Hooks & Remaps
  ;; Ensure we try to install missing grammars when entering a prog-mode
  (add-hook 'prog-mode-hook #'ar/setup-install-grammars)
