...
** File Management
*** Treemacs: Core Configuration and Integrations
A robust, modern, and reliable configuration for the Treemacs file explorer,
designed to mimic the behavior of the Doom Emacs project drawer. It unifies all
related packages to ensure correct load order and a seamless user experience.
#+begin_src emacs-lisp
(use-package treemacs
  ...
  (setq treemacs-collapse-dirs
        (cond
         ((executable-find "python3") 3)
         ((executable-find "python") 3)
         (t 0))) ; Fallback if no python is found
  ;; Disable automatic file following for performance. Use `f` inside treemacs to find manually.
  (setq treemacs-follow-after-init          nil)
  (setq treemacs-is-never-other-window      nil) ; Allow `C-x o` to switch to treemacs
  (setq treemacs-missing-project-action     'remove)
  (setq treemacs-goto-tag-strategy          'ref-history)
...
(use-package treemacs-perspective
  ...
  :config
  ;; Enable perspective integration mode for per-perspective workspaces
  (treemacs-perspective-mode 1))
...
** Version Control
*** Helper Function for Forge
#+begin_src emacs-lisp
(defun ar/init-forge-for-magit ()
  "Load Forge and its Evil Collection bindings.
This is called from `magit-status-mode-hook` to ensure Magit is
fully initialized before Forge tries to add its keybindings."
  (require 'forge)
  ;; Manually run the evil-collection setup for forge now that
  ;; both magit and forge are fully loaded.
...
(use-package magit
  ...
  (add-hook 'magit-status-mode-hook #'ar/init-forge-for-magit))
#+end_src
