...
(use-package cape
  ...
  :init
  ;; === CRITICAL FIX: Proper Order and Method ===
  ;; DON'T use add-to-list repeatedly - it causes order confusion
  ;; Instead, set the default value directly with the CORRECT order
  ;; Order matters: specific capfs first, general ones last
  (setq-default completion-at-point-functions
                (list
                 #'cape-elisp-block     ; FIRST: Elisp in org blocks
                 #'cape-keyword         ; SECOND: Keywords
                 #'cape-file            ; THIRD: File paths
                 #'cape-dabbrev))       ; LAST: Fallback to dabbrev

  ;; === ORG MODE FIX: Ensure buffer-local list ends with 't' ===
  ;; This is the ROBUST fix for org-mode
  (defun ar/setup-org-cape ()
    "Setup completion properly for org-mode buffers."
    ;; Method 1: Ensure local list ends with 't' so global functions run
    (setq-local completion-at-point-functions
                (append (remq t completion-at-point-functions) '(t)))
    
    ;; Method 2: Alternative - explicitly add cape-elisp-block first locally
    ;; Uncomment if Method 1 doesn't work
    ;; (setq-local completion-at-point-functions
    ;;             (cons #'cape-elisp-block
    ;;                   (remq #'cape-elisp-block completion-at-point-functions))))
 
  )
  (add-hook 'org-mode-hook #'ar/setup-org-cape)

  :config
  ;; Make these capfs composable with LSP
