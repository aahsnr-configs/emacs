...
1. THE CHALLENGE:
   ...
   hacks to enable it often cause cursor jumping and severe lag in large files because they interfere with Org's native indentation checks.
...
(use-package org-src-context
  :ensure nil
  ;; Enable the mode globally after Emacs has fully initialized
  :hook (elpaca-after-init . org-src-context-mode)

  :custom
  ;; Set to 't' if you want the edit buffer to show ONLY the current block. Set to 'nil' to see context blocks (read-only) above/below (recommended).
  (org-src-context-narrow-p nil)

  :config
  ;; Enable Apheleia (auto-formatting) inside the edit buffer (C-c ').
  ;; Because org-src-context provides a valid mock file path, Apheleia
  ;; can successfully run external formatters (or Eglot) on this buffer.
