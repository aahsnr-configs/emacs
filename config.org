...
(use-package dockerfile-ts-mode
  ...
  :config
  ;; 2. FIX EGLOT LANGUAGE ID
  ;; Eglot sends "dockerfile-ts-mode" by default. The server rejects this.
  ;; We map it to "dockerfile" so the server accepts the connection.
  (put 'dockerfile-ts-mode 'eglot-language-id "dockerfile")

  ;; 3. DEFINE BASH INJECTION
  (defun ar/dockerfile-ts-setup-injection ()
    "Inject Bash grammar into RUN instructions and reset parser."
    ;; Only attempt injection if Bash grammar is available
    (when (treesit-ready-p 'bash)
      ;; Define the rule: Look for 'shell_command' inside 'run_instruction'
      ;; This correctly skips the --mount flags which are sibling nodes
  ...
  (add-hook 'dockerfile-ts-mode-hook #'ar/dockerfile-ts-setup-injection)

  ;; ;; 4. CONFIGURE LSP SERVER
  ;; ;; Use 'with-eval-after-load' so this runs after Eglot is loaded
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 ;; Use "start" because you installed the official binary via Brew
                 '(dockerfile-ts-mode . ("docker-language-server" "start" "--stdio"))))

  ;; ;; 5. CONFIGURE FORMATTING
  ;; (with-eval-after-load 'apheleia
  ;;   (setf (alist-get 'dockerfile-ts-mode apheleia-mode-alist)
  ;;         '(eglot)))
