...
(use-package hl-todo
  ...
  :hook ((elpaca-after-init . global-hl-todo-mode)
         ;; Register hl-todo as a Flymake backend so TODOs show up in error lists
         (flymake-mode . (lambda ()
  ...
  ;; Re-implement project search using Consult (instead of requiring rg.el)
  ;; This satisfies the "s T" keybinding in your General config.
  (defun hl-todo-rg-project ()
    "Use `consult-ripgrep' to find all TODO keywords in the current project."
    (interactive)
    (if (fboundp 'consult-ripgrep)
        ;; We pass the regex as the initial input to consult-ripgrep
