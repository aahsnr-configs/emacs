...
(use-package apheleia
  ...
  :config
  ;; 1. Enable Apheleia in Org Source Buffers
  ;; This allows manual formatting (C-x C-s) and prepares for the exit hook.
  (add-hook 'org-src-mode-hook #'apheleia-mode)

  ;; 2. Define a Synchronous Formatter for Org Exit
  (defun ar/apheleia-format-on-org-exit (orig-fun &rest args)
    "Run Apheleia formatting synchronously before exiting Org Src mode.
    This prevents the buffer from being killed before the async formatter finishes."
    ;; Only run if Apheleia is enabled in this buffer
    (if (and (bound-and-true-p apheleia-mode)
             (fboundp 'apheleia-format-buffer))
        (progn
          ;; CRITICAL FIX: Use 'call-interactively' to let Apheleia determine 
          ;; the correct formatter argument automatically.
          (call-interactively 'apheleia-format-buffer)
          
          ;; Wait for the async process to complete.
          ;; Apheleia stores the active process in `apheleia--current-process`.
          ;; We wait loop until this process is dead or nil.
          (when (boundp 'apheleia--current-process)
            (let ((proc apheleia--current-process))
              (when (processp proc)
                (while (process-live-p proc)
                  (accept-process-output proc 0.1))))))
      ;; If Apheleia isn't active, do nothing
      )
    
    ;; 3. Proceed with the original exit function (which saves to parent and kills buffer)
    (apply orig-fun args))

  ;; 4. Apply the Advice to the Exit Command
  ;; This ensures formatting happens every time you press C-c ' to save & exit.
  (advice-add 'org-edit-src-exit :around #'ar/apheleia-format-on-org-exit))
#+end_src
