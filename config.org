...
(use-package dirvish
  ...
  ;; Enhanced Evil Bindings with ARROW KEYS
  ;; We use evil-define-key instead of :bind so they persist properly in Evil states
  (with-eval-after-load 'evil
    (evil-define-key 'normal dirvish-mode-map
      ;; Navigation (Arrow Keys)
      (kbd "<left>")  'dired-up-directory    ; Go up one directory
      (kbd "<right>") 'dired-find-file       ; Enter directory / Open file
      (kbd "<up>")    'dired-previous-line   ; Move up
      (kbd "<down>")  'dired-next-line       ; Move down
      
      (kbd "q") 'dirvish-quit
      (kbd "TAB") 'dirvish-subtree-toggle
      
      ;; Layout & History
      (kbd "M-t") 'dirvish-layout-switch     ; Restored your cycle layout binding
      (kbd "F")   'dirvish-layout-toggle     ; Toggle fullscreen
      (kbd "b")   'dirvish-history-go-backward
      (kbd "f")   'dirvish-history-go-forward
      ...
      (kbd "M-e") 'dirvish-emerge-menu       ; Restored emerge menu
      (kbd "M-j") 'dirvish-fd-jump           ; Restored FD jump
...
#+begin_src comment
=============================================================================
  EGLOT & ORG-BABEL INTEGRATION GUIDE
 =============================================================================

  1. THE PROBLEM:
     Language Server Protocol (LSP) servers require a "TextDocument" to have a
     valid URI (usually a file path) to function. They use this path to:
     a) Identify the file type.
     b) locate the "Project Root" (looking up the tree for .git, go.mod, etc.).
     
     When you edit an Org source block (`C-c '`), Org creates a temporary
     buffer that is NOT associated with any file on disk (`buffer-file-name` is nil).
     Consequently, Eglot refuses to start because it cannot construct a valid
     LSP URI and cannot determine where the project root is.

  2. THE SOLUTION (`ar/eglot-org-babel-enable`):
     We define a function that runs immediately after Org sets up the 
     temporary edit buffer. This function:
     
     a) Intercepts the `info` list passed by Org Babel. This list contains:
        (language code-body header-arguments ...)
     
     b) Checks if you defined a `:file` header argument (e.g., used for tangling).
        If yes, it uses that real filename.
     
     c) If no `:file` exists, it constructs a "Fake" Filename:
        "org-src-babel.tmp" inside the *current directory* of the Org file.
     
     d) It forcibly sets `buffer-file-name` to this fake path.
        This satisfies Eglot's requirement for a file path. Crucially, because
        the path is in the *current directory*, Eglot's project detection logic
        (project.el) correctly finds your actual project root (e.g., the .git folder
        in your parent directory), allowing imports and context to work.

  3. EXTENDING SUPPORT:
     To add a new language (e.g., Haskell), simply add "haskell" to the
     `ar/org-babel-supported-languages` list defined below.

  4. DEBUGGING:
     If Eglot fails to start in a source block:
     
     A. Check the Mock Filename:
        - Inside the edit buffer, run: `M-: buffer-file-name`
        - It MUST return a string (e.g., ".../your-project/org-src-babel.tmp").
        - If it returns `nil`, the advice failed to attach.
     
     B. Check Project Detection:
        - Run: `M-: (project-current)`
        - It should return your actual project structure. If it returns nil,
          Eglot assumes single-file mode and some features may fail.

     C. Check Server Logs:
        - Run: `M-x eglot-events-buffer`
        - Look for "TextDocument" errors or server crashes.
 =============================================================================
#+end_src
...
(use-package eglot
  ...
  :hook
  ((prog-mode . (lambda ()
                  ;; Prevent Eglot from running in modes that don't need it
                  ;; or have their own tools (matches init-lsp.txt logic).
  ...
  :config
  ;; -----------------------------------------------------------------------
  ;; Org Babel Integration (Ported from Centaur Emacs init-lsp.el)
  ;; -----------------------------------------------------------------------
  (defun ar/eglot-org-babel-enable (lang)
    "Enable Eglot in Org source blocks for LANG.
    Ported from Centaur Emacs `lsp-org-babel-enable`.
    
    This works by advising the `org-babel-edit-prep:LANG` function
    to initialize Eglot with a mock file context."
    (let* ((edit-pre (intern (format "org-babel-edit-prep:%s" lang)))
           (intern-pre (intern (format "ar/eglot--%s" (symbol-name edit-pre)))))
      
      ;; Define the internal function that initializes Eglot
      (fset intern-pre
            (lambda (info)
              ;; Centaur Logic: Extract :file from info or use default tmp name.
              ;; `info` structure is (language body header-args ...)
              ;; (nth 2 info) gets the header-args list.
              (let* ((header-args (nth 2 info))
                     (tangled-file (alist-get :file header-args))
                     (file-name (or tangled-file "org-src-babel.tmp")))
                
                ;; Set buffer-file-name so Eglot can identify the project.
                ;; We use `expand-file-name` to ensure it resolves to a full
                ;; path inside the current directory.
                (setq-local buffer-file-name (expand-file-name file-name))
                (eglot-ensure))))
      
      ;; Advice logic:
      ;; If the prep function exists (e.g. defined by another package), run ours after it.
      ;; If it doesn't exist, define it as our function.
      (if (fboundp edit-pre)
          (advice-add edit-pre :after intern-pre)
        (fset edit-pre intern-pre))))

  ;; List of languages to enable Eglot for in Org Mode.
  ;; Add new languages here to enable LSP support for them.
  (defconst ar/org-babel-supported-languages
    '("go" "python" "ipython" "ruby" "js" "css" "sass" "c" "rust" "java" "cpp" "c++" "shell" "bash")
    "List of languages to enable Eglot support for in Org Babel.")

  ;; Apply the setup
  (dolist (lang ar/org-babel-supported-languages)
    (ar/eglot-org-babel-enable lang)))
...
(use-package apheleia
  ...
  :config
  ;; We define this so you can map 'eglot to specific languages later.
  (defun ar/apheleia-format-with-eglot (orig scratch callback)
  ...
  ;; Add eglot to the formatter list (but don't set it as default yet)
  (push '(eglot . ar/apheleia-format-with-eglot) apheleia-formatters)

  ;; Best Practice: Inhibit formatting for specific filetypes
  (setq apheleia-inhibit-functions
        (list (lambda () (derived-mode-p 'org-mode 'markdown-mode)))))
#+end_src
