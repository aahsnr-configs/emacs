...
(use-package cape
  ...
  ;; ORG MODE FIX: Ensure cape-elisp-block comes BEFORE yasnippet-capf
  (defun ar/setup-org-cape ()
    "Setup cape-elisp-block with highest priority in org-mode."
    ;; Remove cape-elisp-block if it exists
    (setq-local completion-at-point-functions
                (remq 'cape-elisp-block completion-at-point-functions))
    ;; Add it at the very front (before yasnippet-capf)
    (add-hook 'completion-at-point-functions #'cape-elisp-block -100 t))
  
  (add-hook 'org-mode-hook #'ar/setup-org-cape)

  :config
  ;; Wrap functions for composability
...
(use-package yasnippet-capf
  ...
  (add-hook 'completion-at-point-functions #'yasnippet-capf 100))
#+end_src
