...
#+begin_src emacs-lisp
;; --- Core Toggle Function (for SPC t t) ---
(defun ar/toggle-treemacs-project ()
  "A reliable, Doom-style command to toggle the Treemacs project drawer.
If the Treemacs buffer is visible, close it. Otherwise, open
Treemacs focused on the current project's root."
  (interactive) ; FIX: Add (interactive) to make this a user-callable command.
  (if (treemacs-visible-p)
        (treemacs-close)
    (treemacs-projectile)))

(use-package treemacs
  :defer t
  :after (evil projectile magit perspective winum)
  :bind
  :config
  ;; --- Appearance ---
  (setq treemacs-display-in-side-window          t
        ...
        ;; Use nerd-icons for the theme, provided by treemacs-nerd-icons.
        treemacs-theme                           "nerd-icons")
  ;; --- Behavior ---
  ...
  ;; --- Persistence ---
  (setq treemacs-persist-file (expand-file-name "treemacs-persist" no-littering-var-directory))

  ;; Activate essential modes for a dynamic experience
  ...
  (ar/global-leader
    ...
    "t t" '(ar/toggle-treemacs-project :wk "Treemacs"))
  )

;; --- Treemacs Integrations ---
;; These blocks ensure the necessary extension packages are loaded after Treemacs.
