...
* Remap C-g to Escape
#+begin_src emacs-lisp
;; Use key-translation-map to translate 'C-g' to '<escape>' at the lowest level. This ensures C-g works as Escape in Evil, Minibuffers, Company/Corfu, etc.
(keymap-set key-translation-map "C-g" "<escape>")

;; Since C-g is now Escape, you lose the ability to "Emergency Quit" (unfreeze) Emacs. We map C-M-g (Ctrl+Alt+g) to the real keyboard-quit command just in case.
...
** Show matching parenthesis context when off-screen
#+begin_src emacs-lisp
(use-package paren
  :ensure nil
  ...
  ;; Highlight expression if paren is off-screen
  (show-paren-style 'mixed)
  ;; CRITICAL: Show context when matching paren is off-screen
  ;; 'overlay creates a childframe, t uses echo area
  (show-paren-context-when-offscreen
   (if (childframe-workable-p) 'overlay t)))
#+end_src
...
(use-package hl-todo
  :defer t
  ;; Ensure this custom command is known to Emacs before the package loads, so your 'SPC s T' keybinding works immediately.
  :commands (hl-todo-rg-project)
  
  :custom-face
  ;; Visual Style: Inherit default font, make bold/condensed,
  ;; and explicitly disable the background box (:inverse-video nil).
  (hl-todo ((t (:inherit default :height 0.9 :width condensed :weight bold :underline nil :inverse-video nil))))
  
  :hook ((elpaca-after-init . global-hl-todo-mode)
         ;; Explicitly enable in Org mode so it highlights keywords in
         ;; body text, comments, and lists (not just code blocks).
         (org-mode . hl-todo-mode))
  
  :init
  ;; CRITICAL: This setting prevents hl-todo from clashing with Org headlines.
  ;; Standard Org headlines (* TODO Task) do NOT have a colon.
  ;; hl-todo will only trigger on "TODO:", "FIXME:", etc.
...
(use-package cape
  ...
  :init
  ;; CRITICAL FIX: Use add-hook properly with APPEND argument
  ;; When APPEND is nil (or omitted), functions are added to the FRONT
  ;; When APPEND is non-nil, functions are added to the END
...
(use-package eldoc
  ...
  :custom
  (eldoc-idle-delay 0.1) ;; Fast response like Doom
  ...
  ;; === ORG MODE DOOM-STYLE BREADCRUMBS ===
  (defun ar/org-doom-breadcrumb (&rest _ignored)
    "Return a Doom-style breadcrumb path for the echo area.
    Format: 📄 File › Headline › Subheading"
    (when (and (derived-mode-p 'org-mode)
               (not (org-before-first-heading-p))
               (not (org-in-src-block-p)))
      (let* ((width (window-width))
             ;; 1. Get File/Buffer Name with Icon
             (file-icon (if (and (featurep 'nerd-icons))
                            (nerd-icons-icon-for-mode 'org-mode)
                          "📄"))
             (file-name (buffer-name))
             (root-part (concat 
                         (propertize file-icon 'face '(:inherit font-lock-variable-name-face :height 1.0))
                         " " 
                         (propertize file-name 'face 'font-lock-variable-name-face)))
             
             ;; 2. Get Hierarchy Path
             (path (org-get-outline-path t)) ;; 't' includes current heading
        ...
        (when path
          (concat
           ...
           (mapconcat
            (lambda (heading)
              ;; Dim parent headings, highlight the last one (current)
              (let ((is-last (string= heading (car (last path)))))
                (if is-last
                    (propertize heading 'face 'font-lock-keyword-face 'weight 'bold)
                  (propertize heading 'face 'shadow))))
            path
            sep))))))

  ;; === GENERIC CONTEXT (Global Fallback) ===
  (defun ar/generic-eldoc-context (&rest _ignored)
    "Show current function/section context in generic modes."
  ...
  (add-hook 'eldoc-mode-hook
            (lambda ()
              ;; High priority for Org Breadcrumbs
              (add-hook 'eldoc-documentation-functions #'ar/org-doom-breadcrumb nil t)
              (add-hook 'eldoc-documentation-functions #'ar/generic-eldoc-context nil t))))
...
(global-set-key (kbd "C-c i") #'ar/show-buffer-context)
#+end_src
...
(global-set-key (kbd "C-c =") #'ar/show-char-info)
#+end_src
...
;; Keybinding changed to C-c S (Shift+s) to avoid conflict with Prettify Symbols (C-c s)
(global-set-key (kbd "C-c S") #'ar/show-buffer-state)
#+end_src
