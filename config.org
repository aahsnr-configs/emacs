...
(with-eval-after-load 'tramp
  ...
  ;; Configure PATH to find container tools
  ;; This ensures Emacs sees the tools installed in your Dockerfile
  (connection-local-set-profile-variables
   'container-path
   '((tramp-remote-path . (tramp-own-remote-path
                          "/home/devuser/.local/bin"         ; uv (Python)
                          "/home/devuser/.cargo/bin"         ; Rust
                          "/home/devuser/.bun/bin"           ; Bun
                          "/home/linuxbrew/.linuxbrew/bin"   ; Homebrew
                          "/usr/local/texlive/2025/bin/x86_64-linux")))) ; TeX Live

  ;; Apply these profiles specifically to the "dev-env" container
...
(with-eval-after-load 'eglot
  ...
  (connection-local-set-profile-variables
   'eglot-container
   '((eglot-connect-timeout . 60)      ; Give container time to respond
     (eglot-sync-connect . nil)        ; Don't block Emacs UI
     (eglot-events-buffer-size . 0)))  ; Disable heavy logging

  (connection-local-set-profiles
   '(:application tramp :protocol "podman")
   'eglot-container))

;; Enable Eglot Booster for remote connections
;; Requires 'emacs-lsp-booster' installed in the container
...
(defun ar/podman-run ()
  "Create and run a new persistent development container."
  (interactive)
  (if (ar/podman--exists-p)
    ...
    (let* ((project-root (if (project-current)
           ...
           (command (format "podman run -d --name %s --hostname devbox --userns=keep-id -v %s:%s -p 3000:3000 -p 8080:8080 %s tail -f /dev/null"
                           ar/dev-container-name
