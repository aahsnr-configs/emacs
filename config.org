...
** Podman
#+begin_comment
Now I'll create the final, correct configuration:**Complete rewrite with all corrections:**

## Key Changes:

1. **Pretty-hydra keybindings** - Uses `container-hydra/body` (bound to `C-c C-p`) following your config's style, avoiding the `SPC d` conflict with dired

2. **New dockerfile-ts-mode configuration** - Written from scratch with:
   - Proper eglot language ID mapping
   - Bash injection for syntax highlighting in RUN commands
   - LSP server configuration for docker-langserver

3. **Corrected container functions** - Fixed regex filters using `name=^container-name$` for exact matches (podman filters support regex)

4. **Added useful functions**:
   - `ar/podman-exec` - Run commands in container
   - `ar/podman-logs` - View container logs
   - `ar/podman-status` - Check container status
   - `ar/podman-restart` - Restart container

## Answer: Are Language-Specific Settings Needed?

**NO** - They are **not needed** for basic functionality. Here's why:

- **Eglot works over TRAMP automatically** (since v1.8/Emacs 29+) without any special configuration
- **LSP servers are auto-detected** as long as they're in the PATH (which we configure via `connection-local-variables`)
- Your container already has: basedpyright, typescript-language-server, and texlab installed
- **tramp-own-remote-path** ensures these are found automatically
- Eglot will launch the correct LSP server based on file type, just like it does locally

Language-specific settings would only be needed for advanced scenarios like Python virtual environments or custom LSP server arguments, which aren't necessary for your use case.

**Zero startup impact** - everything deferred until needed.
#+end_comment
...
(with-eval-after-load 'tramp
  ;; Performance optimizations for remote files
  ...
  ;; Enable direct async process for containers (critical for performance)
