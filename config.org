...
(use-package eglot-booster
  ...
  ;; IMPORTANT: Set this to nil to enable boosting over TRAMP
  ;; The Dockerfile already installs emacs-lsp-booster in the container
...
(with-eval-after-load 'tramp
  ...
  ;; CRITICAL FIX: Add all paths where LSP servers are installed
  (connection-local-set-profile-variables
   'container-path
   '((tramp-remote-path . (tramp-own-remote-path    ; Use remote $PATH
                           "/usr/local/bin"          ; Where texlab symlink is
                           "/home/devuser/.cargo/bin" ; Original texlab location
...
(with-eval-after-load 'eglot
  ...
  (connection-local-set-profile-variables
   ...
   '((eglot-connect-timeout . 60)      ; Give container time to respond
     ...
     ;; FIX: Use new config format to suppress "obsolete variable" warning
...
*** Debugging
#+begin_src emacs-lisp
;; Temporarily increase TRAMP verbosity to see what's happening
(setq tramp-verbose 6)

;; Check if texlab is findable
(defun ar/check-texlab ()
  "Check if texlab is findable in the current context."
  (interactive)
  (message "texlab found at: %s" 
           (executable-find "texlab" t)))
#+end_src
  ...
  (add-to-list 'eglot-server-programs
               `((latex-mode LaTeX-mode tex-mode)
                 . ,(lambda (&optional interactive)
                      ;; Find texlab dynamically (works for both local and remote)
                      (list (or (executable-find "texlab" t) ; t = check remote too
