...
- [ ] My emacs config has another issue now. When I press enter inside an org-mode source code block, instead of moving the cursor to the next line, pressing enter moves the cursor to the beginning of the source code block but 1 line after the `#+begin_src "org-babel language"` in the source code block. Pressing the tab key also exhibits the exact same behavior. But I would also have an evil method way of getting to this position without conflicting other keybindings or similarly to the end of the source code block before `#+end_src` as well. There is another weird: when g c to comment or uncomment lines a formatter window is opened like the screenshot.
