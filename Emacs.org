...
(use-package elec-pair
  ...
  :init
  ;; This predicate helps avoid pairing in strings and comments.
  (setq electric-pair-inhibit-predicate 'electric-pair-conservative-inhibit)
  :config
  ;; CRITICAL: This runs *after* the package is loaded.
  ;; It finds the default list of pairs and removes the entry for '<',
  ;; which prevents it from being auto-paired. This is the correct method.
...
(use-package paren
  ...
  :init
  ;; These settings are safe because the mode reads them when it loads.
...
;; THE DEFINITIVE FIX:
;; This advises the `show-paren-mode` function itself. Our code will run
;; just BEFORE the mode is activated, at which point its library MUST have
;; been loaded and its variables (like `show-paren-data`) are guaranteed
;; to exist. This is the most robust way to solve this timing issue.
(defun ar/configure-paren-after-loading (&rest _)
  "Remove '<' from the list of paired delimiters for `show-paren-mode`."
  (setq show-paren-data (assq-delete-all ?< show-paren-data))
  ;; This advice is only needed once, so we remove it for efficiency.
  (advice-remove #'show-paren-mode #'ar/configure-paren-after-loading))

(advice-add #'show-paren-mode :before #'ar/configure-paren-after-loading)
#+end_src
** Aggressive Indent
