...
(use-package tex
  ...
  ;; These MUST be in :custom so they are set before font-latex loads.
  ...
  ;; Modify the "Check" command to use specific flags (-v6 suppresses verbose messages, -H header)
  ...
  (add-hook 'TeX-mode-hook
            (lambda ()
              ;; Tell Emacs how to parse TeX files for spell checking
              (setq-local ispell-parser 'tex)

              ;; Enable word wrapping (visual-line-mode)
              (visual-line-mode 1)

              ;; Don't auto-fill in math blocks
              ;; This prevents breaking equations like $1 + 1 = 2$ across lines.
              ;; We check if texmathp is loaded (it usually is by AUCTeX).
