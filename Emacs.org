...
(use-package project
  ...
  ;; For Emacs 28+, exclude the var directory from project discovery.
  ;; This prevents package repos in elpa from appearing as projects.
  (when (boundp 'project-exclude-directories)
    (add-to-list 'project-exclude-directories no-littering-var-directory))

  ;; For older Emacs versions and as a fallback, add to vc-ignores.
