...
(use-package orderless
  :demand t
  :config
  ;; Doom's custom dispatcher for advanced filtering
  (defun +orderless-consult-dispatch (word _index _total)
    "Orderless dispatcher for consult async commands."
    (cond
     ;; Ensure $ works with Consult tofu suffixes
     ((string-suffix-p "$" word)
      `(orderless-regexp . ,(concat (substring word 0 -1)
                                    (if (fboundp 'consult--tofu-regexp)
                                        (consult--tofu-regexp)
                                      "\\'"))))
     ;; File extensions
  ...
  ;; Doom's style with initialism by default
  ...
  (setq completion-styles '(orderless basic)
        ...
        '((file (styles partial-completion))
          ;; Enable initialism for commands/variables
          (command (styles +orderless-with-initialism))
          (variable (styles +orderless-with-initialism))
          (symbol (styles +orderless-with-initialism)))

        ;; CRITICAL: Doom's escapable split
        orderless-component-separator #'orderless-escapable-split-on-space

        ;; Doom's style dispatchers
        orderless-style-dispatchers
        (list #'+orderless-consult-dispatch
              #'orderless-affix-dispatch))

  ;; Performance: Don't highlight all matches in long candidate lists
  (setq orderless-highlight-matches-in-minibuffer nil))
#+end_src
...
(use-package lsp-bridge
  ...
  (setq acm-enable-doc nil
	...
	acm-backend-lsp-frontend-filter-p nil ;filtering happens in the backend
	acm-backend-lsp-match-mode "prefix"
	acm-enable-search-file-words nil
        acm-backend-search-file-words-enable-fuzzy-match nil)
