...
** Minibuffer Interface with Ivy, Counsel, and Swiper
...
This configuration uses Emacs's modern, built-in project management library,
*project.el*. It provides a fast and integrated foundation for project-related commands, which are further enhanced by the *consult* completion UI.
#+begin_src emacs-lisp
(use-package project
  :defer t
  :config
  ...
  ;; This is the new, more aggressive fix.
  ;; We advise the core project-finding function to completely ignore the `var` directory.
  ;; This prevents it from ever discovering git repos inside `elpa`.
  (defun ar/project-vc-find-function-advice (dir)
    "Advice to prevent project.el from scanning the emacs var directory.
This stops packages in the elpa directory from being treated as projects."
    (not (string-prefix-p (file-name-as-directory no-littering-var-directory)
                          (file-name-as-directory dir))))

  (advice-add 'project-vc-find-function :before-while #'ar/project-vc-find-function-advice))
...
This configures `envrc-mode` to automatically load environment variables
from `.envrc` files when you open a file within a project. It serves as the
critical bridge between `direnv` on the command line and Emacs, ensuring that
LSP clients (eglot), debuggers (dape), and Jupyter kernels automatically use
the correct project-specific environment (e.g., Python virtual environments).

#+begin_src emacs-lisp
(use-package envrc
   :hook (after-init . envrc-global-mode)
  :config
  ;; By default, envrc-mode asks for permission for each new directory.
  ;; For a smoother experience, you can tell it to trust all directories
  ;; that have been previously allowed via the `direnv allow` command.
  (setq envrc-ask-for-permission nil))
#+end_src
...
(use-package treesit
  ...
  (setq major-mode-remap-alist
        '((sh-mode . bash-ts-mode)
          ...
          (gfm-mode . markdown-ts-mode) ; Remap github-flavored markdown as well
...
;   :config
;   ;; Use Tree-sitter for the host buffer (Org mode) for better performance and accuracy.
;   (setq polymode-use-treesitter-for-host-mode t)
;   ;; Disable legacy font-lock for modern Emacs versions.
;   (setq polymode-legacy-font-lock-support nil))
;
; (use-package poly-org
;   :after (polymode org)
;   :hook (org-mode . poly-org-mode)
;   :config
;   ;; Tell poly-org to use Tree-sitter for code blocks when available.
;   (setq poly-org-use-treesitter t))
#+end_src
