...
** LSP: Eglot and Texlab
We integrate *texlab* via Eglot to provide modern LSP features like code actions, smarter completion, and navigation. This runs alongside AUCTeX's features.
#+begin_src emacs-lisp
...
This configures a multi-backend diagnostics system using the built-in flymake. When active, Flymake automatically uses AUCTeX's powerful internal parser to check for compilation errors. We augment this by adding ChkTeX to provide additional style and syntax linting, as recommended by the AUCTeX manual.
...
;; We now enable the built-in ChkTeX support from AUCTeX.
;; Make sure `chktex` is installed and available in your system's PATH.
...
This configures apheleia to use the latexindent command-line tool for formatting LaTeX source code, ensuring consistent and clean documents. It works for both standalone .tex files and for LaTeX source blocks inside Org mode.
#+begin_src emacs-lisp
(with-eval-after-load 'apheleia
  ...
  ;; For Org mode, add a formatter for "latex" source blocks.
  (add-to-list 'apheleia-formatters-alist
               '((latex . ("latexindent" "-g" "/dev/null")))))
#+end_src

** Completion Framework Integration
*Are you sure about the eglot completion and AucTeX completion working together*
#+begin_src emacs-lisp
(defun ar/latex-completion-setup ()
  "Add AUCTeX completion backend to the local completion functions."
  ;; Add AUCTeX's function to the buffer-local list of completion backends.
...
** 📥 TODO Citation Ecosystem
This section configures a powerful citation workflow centered around Zotero.
citar is the main interface, and it is configured to use the .bib files provided
by the .dir-locals.el setup, which are kept in sync by Zotero. *I will be using
direnv, but still keep integrated*
#+begin_src emacs-lisp
(use-package reftex
  :ensure nil
  :after tex
