...
;; GC when idle to prevent interruptions during active editing
(run-with-idle-timer 5 t #'garbage-collect)

;; GC when losing focus (Emacs is in background)
...
(use-package org
  ...
  ;; Display settings
  (org-hide-leading-stars nil)  ;; CHANGED: Faster without hiding
  (org-hide-emphasis-markers nil)  ;; CHANGED: Show for performance
