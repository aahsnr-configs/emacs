...
;; Restore GC settings after Elpaca finishes loading all packages
...
;; CRITICAL: Wait for elpaca-use-package to be ready before using :ensure keyword
(elpaca-wait)

;; NOW set the queue limit after elpaca-use-package-mode is enabled
;;(setq elpaca-queue-limit 1)

;; Configure use-package behavior
(eval-and-compile
  (setq use-package-verbose nil  ;; Reduce startup messages
        use-package-expand-minimally t
        use-package-compute-statistics nil  ;; Disable unless profiling
...
(use-package evil-matchit
  :after evil
  :hook (elpaca-after-init . global-evil-matchit-mode))
...
(use-package evil-mc
  ...
  :config
  ;; Set up standard bindings that mimic the old hydra behavior
  ;; "gz" is the default evil-mc prefix
  (evil-define-key 'normal 'global
    (kbd "g z m") #'evil-mc-make-all-cursors
    (kbd "g z n") #'evil-mc-make-and-goto-next-match
    (kbd "g z p") #'evil-mc-make-and-goto-prev-match
    (kbd "g z j") #'evil-mc-make-cursor-move-next-line
    (kbd "g z k") #'evil-mc-make-cursor-move-prev-line))
#+end_src
...
;; CRITICAL: For daemon mode, wait until after Elpaca loads packages
(if (daemonp)
    (with-eval-after-load 'elpaca
      (add-hook 'elpaca-after-init-hook
                (lambda ()
...
(use-package aggressive-indent
  ...
  :init
  ;; CRITICAL FIX: Disable aggressive-indent in Evil Visual State to prevent
  ;; text jumping while selecting blocks.
  (defun ar/disable-aggressive-indent-in-visual ()
    (when (bound-and-true-p aggressive-indent-mode)
      (aggressive-indent-mode -1)
      (setq-local ar/aggressive-indent-was-on t)))

  (defun ar/enable-aggressive-indent-after-visual ()
    (when (bound-and-true-p ar/aggressive-indent-was-on)
      (aggressive-indent-mode 1)
      (kill-local-variable 'ar/aggressive-indent-was-on)))

  (add-hook 'evil-visual-state-entry-hook #'ar/disable-aggressive-indent-in-visual)
  (add-hook 'evil-visual-state-exit-hook #'ar/enable-aggressive-indent-after-visual)
...
(use-package symbol-overlay
  ...
  ;; Define helpers in :init so they exist even if symbol-overlay isn't loaded yet
  (defun turn-off-symbol-overlay (&rest _)
    "Turn off symbol highlighting if it is currently active."
    (interactive)
    (when (bound-and-true-p symbol-overlay-mode)
      (symbol-overlay-mode -1)))

  (defun turn-on-symbol-overlay (&rest _)
    "Turn on symbol highlighting if applicable."
    (interactive)
    (when (derived-mode-p 'prog-mode 'yaml-mode 'yaml-ts-mode)
      (if (fboundp 'symbol-overlay-mode)
          (symbol-overlay-mode 1)
        ;; If function is void, the package isn't loaded, so we can ignore
        (message "Symbol overlay not loaded, skipping enable."))))

  :config
  ;; Safe advice for easy-kill
...
(use-package tabspaces
  ...
  (tabspaces-session t)
  (tabspaces-session-auto-restore nil)  ; Disable auto-restore to prevent marker errors
  ...
  (with-no-warnings
    ...
    ;; Delete child frames before saving session
    ;; This prevents posframe popups from being saved in the session
    ...
    ;; Clean up any markers before saving
    (defun my-tabspaces-clean-markers (&rest _)
      "Clean up dead markers before saving session."
    ...
    ;; Bury Messages buffer after restoring session
    ;; Use idle timer to ensure restoration is complete
    ...
    ;; Safe session restoration wrapper
    (defun my-tabspaces-safe-restore ()
      "Safely restore tabspaces session with error handling."
      (interactive)
      (condition-case err
          (progn
            (message "Restoring tabspaces session...")
            (tabspaces-restore-session)
            (message "Tabspaces session restored successfully."))
        (error
         (message "Tabspaces restore failed: %s. Starting with default workspace." 
                  (error-message-string err))
         ;; Create default workspace if restoration fails
         (ignore-errors
           (tab-bar-new-tab)
           (tab-bar-rename-tab "Default"))
         nil)))

    ;; Safe session save wrapper
    (defun my-tabspaces-safe-save ()
      "Safely save tabspaces session with error handling."
      (interactive)
      (condition-case err
          (progn
            ...
            (message "Tabspaces session saved successfully."))
        (error
    ...
    (advice-add 'tabspaces-save-session :around
                (lambda (orig-fun &rest args)
                  ...
                  (condition-case err
                      (apply orig-fun args)
                    (error
                     (message "Error saving tabspaces: %s" (error-message-string err))
                     nil))))

    ;; Add advice to restore function
    ...
    ;; Replace auto-restore with manual safe restore
    ;; Delay restoration to ensure all packages are loaded
    (add-hook 'elpaca-after-init-hook
              (lambda ()
                (run-with-idle-timer 2 nil #'my-tabspaces-safe-restore))
              90)))  ; Run after other startup hooks
#+end_src
...
(use-package persistent-scratch
  ...
  :hook ((elpaca-after-init . persistent-scratch-autosave-mode)
